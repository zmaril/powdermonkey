...
Some genuinely-finished work never lands a trailer (a squash that dropped it, out-of-band work, a won't-do item, a phase-less task), so reconciliation can never close it. For those the operator can mark a phase/task done by hand; a `done_source` column (`reconciled` | `operator`) records which path completed each row, so an asserted completion is first-class but never disguised as work that landed on `main`. See [docs/completion-model.md](docs/completion-model.md).
