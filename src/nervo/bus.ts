...
//
// Dispatch is the one door every state change goes through. Given a host, an entity,
// and an action id, the bus: resolves the action, checks it targets the entity's kind,
// runs the permission spec, prices the cost spec, computes the target state, validates
// that move against the entity's FSM, and — only if all of that holds — emits a
// `Transaction`: the before/after entities and the cost. It never mutates anything; the
// caller (or history.ts) owns the entity and applies `tx.after`. That purity is what
// makes undo trivial: a transaction already carries its own `before`.
 ...
 *  action, wrong entity kind, denied by the permit, or an illegal FSM move). */
...
export function dispatch(
  ...
  const to = action.target(ctx);
  if (!canTransition(fsm, entity.state, to)) {
    return { ok: false, error: `illegal transition ${entity.state} → ${to}` };
  }

  const cost = action.cost ? action.cost(ctx) : NO_COST;
  const after: Entity = { ...entity, state: to };
 ...
 *  whose permit passes and whose target is a legal FSM edge. This is what a co-pilot pane
 *  or action menu renders — the affordances, already filtered by permission and lifecycle. */
export function available(host: HostContract, entity: Entity): string[] {
  ...
  for (const action of host.actions) {
    ...
    if (canTransition(fsm, entity.state, action.target(ctx))) out.push(action.id);
  }
  return out;
}
