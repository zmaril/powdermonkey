 ...
 *  a tmux server pm doesn't own — e.g. disponent's `-L <endpoint>` socket, where a
 *  local-backend agent runs (`dsp-<uid>`) — while `tmux()` below stays pinned to
 *  pm's own private socket. */
