...
export type ApplyChangesResult =
  | { ok: true; appliedCount: number; remaining: number; proposal: Proposal }
  | { ok: false; error: string };

/** Apply a SUBSET of a proposal's changes (by index) and CONSUME them — the applied
 *  changes are removed from the proposal's change-set. This is what powers immediate,
 *  per-change accept from the review gutter: each accepted change lands as a typed
 *  mutation (so a created node gets a real id, no whole-markdown churn), and because
 *  it's removed from the set, re-projecting the proposal afterwards stays consistent.
 *  When the last change is consumed the proposal flips to `applied`. */
export async function applyProposalChanges(
  ...
  const proposal = await getProposal(id);
  if (!proposal) return { ok: false, error: "proposal not found" };
  if (proposal.status === ProposalStatus.Applied || proposal.status === ProposalStatus.Rejected)
    return { ok: false, error: `proposal is ${proposal.status}` };

  const changes = proposal.changes;
  const sel = new Set(indices.filter((i) => i >= 0 && i < changes.length));
  if (sel.size === 0) return { ok: false, error: "no valid change indices" };

  const remaining = changes.filter((_, i) => !sel.has(i));
  const now = new Date();
  const updated = await db.transaction(async (tx) => {
    const refs = new Map<string, number>();
    for (let i = 0; i < changes.length; i++) {
      if (sel.has(i)) await applyChange(tx, changes[i], refs, now);
    }
  ...
  return { ok: true, appliedCount: sel.size, remaining: remaining.length, proposal: updated };
}
