...
// Prove the holder-protocol client speaks disponent's wire format byte-for-byte
// (transcribed from crates/disponent-hold/src/protocol.rs): a `{"v":1}\n`
// handshake, then `1 byte kind | LE u32 len | payload` frames — server Data(0) /
// Heartbeat(1) / Exit(2, 5-byte disposition+LE i32), client Input(0) / Resize(1,
// u16 LE cols,rows) / Detach(2). The test stands up a fake holder on a unix socket.
...
test("holder client: handshake, decodes Data/Exit, encodes Input/Resize", async () => {
  try {
    unlinkSync(sock);
  } catch {}
  ...
  const server = Bun.listen({
    ...
    socket: {
      open(s) {
        conn = s;
        // Handshake line first, then a Data frame carrying scrollback.
        s.write(new TextEncoder().encode('{"v":1}\n'));
  ...
  // Client → server: Input("hi") then Resize(80, 24).
  client.write("hi");
  client.resize(80, 24);
  await sleep(50);
  ...
  expect(received).toEqual([...input, ...resize]);
