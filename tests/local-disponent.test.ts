...
test("teleport opts route through disponent: fetchRemote + startup map to the dispatch spec", async () => {
  ...
  try {
    ...
    // …and the teleport opts landed on the dispatch spec: branch→gitRef,
    // fetchRemote→fetchRemote (#74), startup→agentCmd verbatim (#33).
    ...
    // spec is identical to before this change.
